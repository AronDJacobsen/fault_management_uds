...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # LSTM forward pass
        out, (hn, cn) = self.lstm(x, (h0, c0))  # 'out' contains all hidden states
...
class TransformerModel(nn.Module):
    def __init__(self, input_size, sequence_length, hidden_size, output_size, num_heads, num_layers, dropout):
        ...
        self.returns = ['prediction', 'final_hidden', 'max_attention']
    ...
    def forward(self, x):
        ...
        # Initialize a list to store attention weights from each layer
        attention_weights = []

        # Pass through the transformer encoder
        for layer in self.transformer_encoder.layers:
            # Forward pass through the layer, return attention weights as well
            x, attn_weights = layer(x, src_mask=None)  # src_mask is optional, adjust based on your needs
        ...
        # For simplicity, consider high attention weights as indicating potential outliers
        max_attention = [attn_weights.max(dim=-1)[0] for attn_weights in attention_weights]

        return prediction, final_hidden, max_attention
